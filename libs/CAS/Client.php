    ...
    */
    ...
    private function _getServerBaseURL()
    {
        ...
        if ( empty($this->_server['base_url']) ) {
            //$this->_server['base_url'] = 'https://' . $this->_getServerHostname();   //ison.zhang
    ...
    public function handleLogoutRequests($check_client=true, $allowed_clients=false)
    {
        ...
        if ($check_client) {
            ...
            foreach ($allowed_clients as $allowed_client) {
            	 if (($client == $allowed_client) or ($client_ip == $allowed_client) or (SERVER_IP == $client_ip)) {
        ...
        if ($allowed) {
            ...
            if ($this->getChangeSessionID()) {
            	...
            	//$session_id = session_id();  		// ison.zhang
    ...
    private function _renameSession($ticket)
    {
        ...
        if ($this->getChangeSessionID()) {
            if (!empty($this->_user)) {
                ...
                setcookie("cassession", $session_id, time()+3600, '/');            //ison.zhang
                setcookie("casflag", $session_id, time()+3600, '/');           	 //ison.zhang
            } else {
